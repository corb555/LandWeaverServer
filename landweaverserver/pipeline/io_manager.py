...
from landweaverserver.common.keys import SourceKey, SourceRndrSpec
from landweaverserver.pipeline.job_control import JobManifest
from landweaverserver.pipeline.shared_memory import SlotRegistry
...
class IOSystem:
    ...
    @staticmethod
    def get_anchor_windows(manifest: 'JobManifest', anchor_key: 'SourceKey') -> List[Window]:
        """
        ...
        """
        path = manifest.resources.sources[anchor_key]
...
class IOManager:
    ...
    def __init__(self, render_cfg: Any, sources: Dict[Any, Path], anchor_key: Any):
        self.render_cfg = render_cfg
        self.anchor_key = anchor_key

        # 1. Store the PATHS in a separate attribute
    ...
    def __enter__(self):
        ...
        for dkey, path in self.source_paths.items():
            try:
                # 4. Open and store in the handles dictionary (self.sources)
    ...
    def read_source_block_ref(
        ...
        # 1. SPATIAL SETUP
        drv_rndr_spec: SourceRndrSpec = self.render_cfg.get_spec(key)
        ...
        try:
            ...
            raise RuntimeError(
                f"SHM Pool Exhausted for source '{key}'. "
            )
