...
from landweaverserver.common.ipc_packets import (RenderPacket, SourceBlockRef, BlockReadPacket, WindowRect, Envelope, \
                                                 Op)
...
class TileDispatcher:
    ...
    Responsibilities:
        ...
        - Allocate and track source block references
    ...
    def dispatch_next_tile(self, job_id: str) -> DispatchResult:
        ...
        for source_id in manifest.resources.sources:
            slot_id, is_cached = self.resources.registry.get_or_allocate(source_id, window)
    ...
    def on_source_block_loaded(
        ...
        RenderPacket]:
        """Record one loaded source block and return a tile when ready.

        Args:
            job_id: Active job identifier.
            tile_id: Tile whose block has finished loading.
            read_duration: Time spent loading this source block.
    ...
    def on_tile_written(self, tile_id: int) -> bool:
        ...
        for source_key, ref in finished_tile["block_map"].items():
            self.resources.registry.release(source_key, ref.slot_id)
