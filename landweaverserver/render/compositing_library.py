...
class CompositingLibrary:
    ...
    @staticmethod
    @register_op("create_buffer", required_attrs=["buffer"])
    def create_buffer(buffers, surfaces, factors, factor, spec, ctx):
        # 1. Option A: Create from a constant color (Tuning Mode)
        # Check if 'color' is provided in the YAML params (e.g., [255, 255, 255])
        fill_color = spec.params.get("color")

        if fill_color is not None:
            # Create a solid RGB block of the target shape
            # We use the same float32 standard as the rest of the engine
            buffers[spec.buffer] = np.full(
                (*ctx.target_shape, 3), fill_color, dtype=np.float32
            )
            return

        # 2. Option B: Create from an existing surface (Standard Mode)
        if not spec.input_surfaces:
            raise ValueError("create_buffer: No input_surfaces or 'color' param provided.")
    ...
    def apply_zonal_gradient(buffers, surfaces, factors, factor, spec, ctx):
        ...
        if mask is not None and factor is not None:
            # color_0 matches factor=0, color_1 matches factor=1
            c0 = np.array(spec.params["color_0"], dtype=np.float32).reshape(1, 1, 3)
            c1 = np.array(spec.params["color_1"], dtype=np.float32).reshape(1, 1, 3)

            gradient = (c0 * (1.0 - factor)) + (c1 * factor)

            # Masked replace
            buffers[spec.buffer] = np.where(mask > 0.1, gradient, current)
        else:
            if mask is None:
                available = factors.keys()
                raise ValueError(
                    f"Error apply_zonal_gradient - Mask '{spec.mask_nm}' Not Found. Available: "
                    f"{available}"
                )
            else:
                raise ValueError(
                    f"Error apply_zonal_gradient - Factor '{spec.factor}' Not Found."
                )
