...
from landweaverserver.common.keys import SurfaceKey, _BlendSpec
from landweaverserver.render.compositing_library import COMPOSITING_REGISTRY
...
class CompositingEngine:
    ...
    def _condition_factor(spec: _BlendSpec, factors: dict, step_idx: int) -> Optional[np.ndarray]:
        ...
        factor = factors.get(spec.factor)
        if factor is None:
            available = list(factors.keys())
            raise KeyError(
                f"Step {step_idx}: factor_spec '{spec.factor}' missing. Available: {available}"
            )
    ...
    def _log_pipeline_error(e: Exception, index: int, spec: Any, buffers: dict) -> None:
        ...
        # Signal telemetry
        print(f"Factor:       {spec.factor or 'None'}")
