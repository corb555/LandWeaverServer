        ...
        while True:
            ...
            match envelope.op:
                case Op.LOAD_BLOCK:
                    ...
                    #print(f"[{section}] NEW TILE {packet.tile_id} ====================")
                    ...
                    try:
                        ...
                        print(
                            f"❌ {section} Missing source file for job '{packet.job_id}', "
                            f"driver '{packet.driver_id.value}': {exc}"
                        )
                        payload = ErrorPacket(
                            job_id=packet.job_id, tile_id=packet.tile_id, stage=section,
                            message=f"{section} Missing source file for '"
                                    f"{packet.driver_id.value}': {exc}", )
                        send_error(status_q, payload)

                    except OSError as exc:
                        print(
                            f"❌ {section} File access error for job '{packet.job_id}', "
                            f"driver '{packet.driver_id.value}': {exc}"
                        )
                        payload = ErrorPacket(
                            job_id=packet.job_id, tile_id=packet.tile_id, stage=section,
                            message=f"{section} File access error for '{packet.driver_id.value}':"
                                    f" {exc}", )
                        send_error(status_q, payload)
                ...
                case _:
                    payload = ErrorPacket(
                        job_id="unknown", tile_id=-1, stage=section,
