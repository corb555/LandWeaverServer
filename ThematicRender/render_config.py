...
class RenderConfig:
        ...
        try:
            ...
            for dname, data in defs.get("driver_specs", {}).items():
                ...
                driver_specs[dkey] = DriverRndrSpec(
                    ...
                    halo_px=int(data.get("halo_px", 64)),
            ...
            # Path resolution done separately
            resolved_files = {}

            return cls(
                logic=defs.get("drivers", {}), specs=driver_specs, files=resolved_files,
                raw_defs=defs, pipeline=pipeline, factors=factors, surfaces=surfaces, noises=noises,
                modifiers=modifiers
            )
    ...
    def resolve_paths(self, prefix: str, build_dir: Path, output_file: str) -> None:
        ...
        Uses parameters from the Render Command to finalize the file dictionary.
        """
        resolved_files = {}

        # 1. Standard Files (Direct paths)
        for k, v in self.raw_defs.get("files", {}).items():
            resolved_files[k] = Path(v).expanduser()

        # 2. Prefixed Files (Build artifacts like ramps/masks)
        # Often used for files generated during the build process
        for k, v in self.raw_defs.get("prefixed_files", {}).items():
            resolved_files[k] = build_dir / f"{prefix}{v}"

        # 3. Output Destination
        # If output_file is an absolute path, use it; otherwise, join with build_dir
        out_path = Path(output_file)
        if not out_path.is_absolute():
            out_path = build_dir / out_path

        resolved_files["output"] = out_path.expanduser()
