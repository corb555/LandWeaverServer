...
class IOManager:
    ...
    def read_into_shm(
        ...
        src.read(
            1,
            window=geom.read_window,
            boundless=True,
            fill_value=fill,
            out=out_data[0, :h, :w] # <--- Crucial: slice 2D destination
        )

        if key == DriverKey.THEME:
            # Check what we just read into the buffer
            vals_read = np.unique(out_data[0, :h, :w])
    ...
    def read_driver_block_ref(
        ...
        try:
            ...
            return pool.write(
                ...
                pad_data=(float(fill) if drv_rndr_spec.dtype != np.uint8 else int(fill)), pad_mask=0.0
            )
        except Exception:
