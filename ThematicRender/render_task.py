    ...
    try:
        while True:
            ...
            ctx = sync_ctx_for_packet(
                ctx=ctx, packet_job_id=packet.job_id,
                shm_store=shm_store, load_ctx=load_worker_job_ctx, err_prefix="WORKER"
            )
            ...
            match envelope.op:
                ...
                case _:
                    payload = ErrorPacket(
                        packet.job_id, -1, section, f"{section} Unknown message rcvd"
                    )
                    send_error(status_q, payload)
    except Exception as e:
        print(f"{section} ERRROR {e}")
        traceback.print_exc()
        sys.exit(1)
...
class RenderWorkspace:
    ...
    def sync_to_context(self, ctx: WorkerContext):
        ...
        if res.logic_hash != self.current_logic_hash or self.current_logic_hash is None:
            print(f"⚙️ [Workspace] HASH change detected sync_to_context Hash: {res.logic_hash[:8]}")
            ...
            self.factor_eng = FactorEngine(
                ctx.render_cfg,
                ctx.themes,
                noise_lib, # Pass the attached library
                ctx.render_cfg.factors,
                res,
                None
            )
            print("HASH rebuilt factor_eng")
...
def render_task(*, packet, ctx, workspace, out_pool, pool_map):
    ...
    try:
        ...
        print(f"RNDR ERROR {e}")
        out_pool.release(out_slot)
        raise e


def ZZsetup_render_job(ctx: WorkerContext, job_id) -> None:
    """Performs render-time initialization for procedural engines."""
    # Load QML -> LUT
    # Scan FactorSpecs to see if any categorical themes are required
    #print("SETUP RENDER JOB")
    ...
    if hasattr(ctx.factors_engine, 'theme_reg'):
        ctx.factors_engine.theme_reg.load_metadata(ctx.render_cfg)
        ctx.factors_engine.theme_reg.load_theme_style()

    # Create surfaces by interpolating Color Ramp definitions (elev, RGB)
    ctx.surfaces_engine.load_surface_ramps(ctx.resources)

    water_color = ctx.themes._id_to_color.get(4)
    #print(f"TRACE [Worker {job_id}]: Dictionary received for ID 4: {water_color}")
